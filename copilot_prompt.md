...
- In `GxsChannelDialog::groupInfoToGroupItemInfo`, replace:
  ...
  with:
  ```cpp
  RsGxsGroupId grpId = channelGroupData->mMeta.mGroupId;
  groupItemInfo.icon = GxsIdDetails::makeDefaultGroupIcon(RsGxsId(grpId), ":icons/channel.png", GxsIdDetails::ORIGINAL);
  ```
...
- In `PostedDialog::groupInfoToGroupItemInfo`, replace the static `:icons/png/postedlinks.png` with:
  ```cpp
  RsGxsGroupId grpId = postedGroupData->mMeta.mGroupId;
  groupItemInfo.icon = GxsIdDetails::makeDefaultGroupIcon(RsGxsId(grpId), ":icons/board.png", GxsIdDetails::ORIGINAL);
  ```
...
- In `WikiDialog::GroupMetaDataToGroupItemInfo`, replace:
  ...
  with:
  ```cpp
  RsGxsGroupId grpId = groupInfo.mGroupId;
  groupItemInfo.icon = GxsIdDetails::makeDefaultGroupIcon(RsGxsId(grpId), ":icons/wiki.png", GxsIdDetails::ORIGINAL);
  ```
