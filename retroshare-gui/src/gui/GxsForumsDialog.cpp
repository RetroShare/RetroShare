...
//#AFTER MERGE	setTextColorNotSubscribed(Qt::black);
//#AFTER MERGE	setTextColorUnread(Qt::black);
//#AFTER MERGE	setTextColorUnreadChildren(Qt::gray);
//#AFTER MERGE	setTextColorRead(Qt::gray);
//#AFTER MERGE	setTextColorMissing(Qt::darkRed);
...
void GxsForumsDialog::changeEvent(QEvent *e)
{
...
void GxsForumsDialog::filterColumnChanged(int column)
{
	if (m_bProcessSettings) {
		return;
	}
...
bool GxsForumsDialog::convertMsgToThreadWidget(const RsGxsForumMsg &msgInfo, bool useChildTS, uint32_t filterColumn, GxsIdTreeWidgetItem *item)
{
	QString text;
...
void GxsForumsDialog::loadForumBaseThread(const RsGxsForumMsg &msg)
{
	...
	GxsIdTreeWidgetItem *item = new GxsIdTreeWidgetItem(threadCompareRole); // no Parent.
...
void GxsForumsDialog::loadForumChildMsg(const RsGxsForumMsg &msg, QTreeWidgetItem *parent)
{
	...
	{
		child = new GxsIdTreeWidgetItem(threadCompareRole); // no Parent.
	}
	else
	{
		child = new GxsIdTreeWidgetItem(threadCompareRole, parent);
	}
