...
#include "rshare.h"
#include "GxsCommentsItem.h"
#include "ui_GxsCommentsItem.h"
...
GxsCommentsItem::GxsCommentsItem(FeedHolder *feedHolder, uint32_t feedId, const RsGroupMetaData& group_meta, const RsGxsMessageId &messageId, bool isHome, bool autoUpdate,const std::set<RsGxsMessageId>& older_versions) :
...
GxsCommentsItem::GxsCommentsItem(FeedHolder *feedHolder, uint32_t feedId, const RsGxsGroupId& groupId, const RsGxsMessageId &messageId, bool isHome, bool autoUpdate,const std::set<RsGxsMessageId>& older_versions) :
...
void GxsCommentsItem::paintEvent(QPaintEvent *e)
{
...
GxsCommentsItem::~GxsCommentsItem()
{
	delete(ui);
}

bool GxsCommentsItem::isUnread() const
{
	return IS_MSG_UNREAD(mPost.mMeta.mMsgStatus) ;
}

void GxsCommentsItem::setup()
{
	/* Invoke the Qt Designer generated object setup routine */

	ui = new Ui::GxsCommentsItem;
	ui->setupUi(this);
...
bool GxsCommentsItem::setPost(const RsGxsChannelPost &post, bool doFill)
{
	if (groupId() != post.mMeta.mGroupId || messageId() != post.mMeta.mMsgId) {
		std::cerr << "GxsCommentsItem::setPost() - Wrong id, cannot set post";
		std::cerr << std::endl;
		return false;
	}
...
QString GxsCommentsItem::getTitleLabel()
{
	return QString::fromUtf8(mPost.mMeta.mMsgName.c_str());
}

QString GxsCommentsItem::getMsgLabel()
{
...
QString GxsCommentsItem::groupName()
{
...
void GxsCommentsItem::loadMessage()
{
#ifdef DEBUG_ITEM
	std::cerr << "GxsCommentsItem::loadMessage()";
	std::cerr << std::endl;
#endif
	RsThread::async([this]()
	{
		...
		{
#ifdef DEBUG_ITEM
			std::cerr << "GxsCommentsItem::loadMessage() Wrong number of Items. Remove It.";
			std::cerr << std::endl;
#endif
...
void GxsCommentsItem::loadComment()
{
#ifdef DEBUG_ITEM
	std::cerr << "GxsCommentsItem::loadComment()";
	std::cerr << std::endl;
#endif
...
void GxsCommentsItem::fillExpandFrame()
{
...
QString GxsCommentsItem::messageName()
{
	return QString::fromUtf8(mPost.mMeta.mMsgName.c_str());
}

void GxsCommentsItem::setReadStatus(bool isNew, bool isUnread)
{
	if (isNew)
...
void GxsCommentsItem::doExpand(bool open)
{
	if (mFeedHolder)
	{
...
void GxsCommentsItem::expandFill(bool first)
{
...
void GxsCommentsItem::readAndClearItem()
{
#ifdef DEBUG_ITEM
	std::cerr << "GxsCommentsItem::readAndClearItem()";
	std::cerr << std::endl;
#endif
	readToggled(false);
	removeItem();
...
void GxsCommentsItem::unsubscribeChannel()
{
#ifdef DEBUG_ITEM
	std::cerr << "GxsCommentsItem::unsubscribeChannel()";
	std::cerr << std::endl;
#endif
...
void GxsCommentsItem::readToggled(bool /*checked*/)
{
	if (mInFill) {
		return;
	}
...
void GxsCommentsItem::makeDownVote()
{
...
void GxsCommentsItem::makeUpVote()
{
	RsGxsGrpMsgIdPair msgId;
	msgId.first = mPost.mMeta.mGroupId;
	msgId.second = mPost.mMeta.mMsgId;
