...
class CompressedChunkMap
{
	public:
		...
		uint32_t countFilledChunks(uint32_t nbchks) const
		{
			uint32_t res = 0 ;
		...
		uint64_t computeProgress(uint64_t total_size,uint32_t chunk_size) const
		{
			...
			if(residue && operator[](nbchks-1))
				return (countFilledChunks(nbchks)-1)*chunk_size + (total_size%chunk_size) ;
			else
				return countFilledChunks(nbchks)*chunk_size ;
		}
