...
bool RsDiscSpace::checkForDiscSpace(RsDiscSpace::DiscLocation loc)
{
	...
	if(_last_check[loc]+DELAY_BETWEEN_CHECKS < now)
	{
		...
		switch(loc)
		{
		    ...
		    case RS_CONFIG_DIRECTORY: 		rs = crossSystemDiskStats(RsAccountsDetail::instance().PathAccountDirectory().c_str(),free_blocks,block_size) ;
#ifdef DEBUG_RSDISCSPACE
													std::cerr << "  path = " << RsInit::RsConfigDirectory() << std::endl ;
#endif
													break ;

		    case RS_PGP_DIRECTORY: 		   rs = crossSystemDiskStats(RsAccountsDetail::instance().PathPGPDirectory().c_str(),free_blocks,block_size) ;
#ifdef DEBUG_RSDISCSPACE
