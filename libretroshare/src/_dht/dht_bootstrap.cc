...
pqiConnectCbStun::pqiConnectCbStun()
{
}
...
void pqiConnectCbStun::printPeerStatus()
{
    RsStackMutex stack(peerMtx); /**** LOCK MUTEX ***/
...
void pqiConnectCbStun::stunStatus(std::string id, struct sockaddr_in raddr, uint32_t type, uint32_t flags)
{
    addPeer(id);
    {
        RsStackMutex stack(peerMtx); /**** LOCK MUTEX ***/
...
void pqiConnectCbStun::stunSuccess(std::string id, struct sockaddr_in toaddr, struct sockaddr_in ansaddr)
{
