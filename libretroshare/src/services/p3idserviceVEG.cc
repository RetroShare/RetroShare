...
RsIdentityVEG *rsIdentityVEG = NULL;
...
p3IdServiceVEG::p3IdServiceVEG(uint16_t type)
	:p3GxsDataServiceVEG(type, new IdDataProxy()), mIdMtx("p3IdService"), mUpdated(true)
{
...
int	p3IdServiceVEG::tick()
{
	//std::cerr << "p3IdServiceVEG::tick()";
	//std::cerr << std::endl;
...
bool p3IdServiceVEG::updated()
{
	...
	return false;
       ...
       /* Data Requests */
bool p3IdServiceVEG::requestGroupInfo(     uint32_t &token, uint32_t ansType, const RsTokReqOptionsVEG &opts, const std::list<std::string> &groupIds)
{
	generateToken(token);
	std::cerr << "p3IdServiceVEG::requestGroupInfo() gets Token: " << token << std::endl;
	storeRequest(token, ansType, opts, GXS_REQUEST_TYPE_GROUPS, groupIds);

	return true;
}

bool p3IdServiceVEG::requestMsgInfo(       uint32_t &token, uint32_t ansType, const RsTokReqOptionsVEG &opts, const std::list<std::string> &groupIds)
{
	generateToken(token);
	std::cerr << "p3IdServiceVEG::requestMsgInfo() gets Token: " << token << std::endl;
	storeRequest(token, ansType, opts, GXS_REQUEST_TYPE_MSGS, groupIds);

	return true;
}

bool p3IdServiceVEG::requestMsgRelatedInfo(uint32_t &token, uint32_t ansType, const RsTokReqOptionsVEG &opts, const std::list<std::string> &msgIds)
{
	generateToken(token);
	std::cerr << "p3IdServiceVEG::requestMsgRelatedInfo() gets Token: " << token << std::endl;
	...
	return true;
        ...
        /* Generic Lists */
bool p3IdServiceVEG::getGroupList(         const uint32_t &token, std::list<std::string> &groupIds)
{
	uint32_t status;
	uint32_t reqtype;
	uint32_t anstype;
	...
	{
		std::cerr << "p3IdServiceVEG::getGroupList() ERROR AnsType Wrong" << std::endl;
		return false;
	}
	
	if (reqtype != GXS_REQUEST_TYPE_GROUPS)
	{
		std::cerr << "p3IdServiceVEG::getGroupList() ERROR ReqType Wrong" << std::endl;
		return false;
	}
	
	if (status != GXS_REQUEST_STATUS_COMPLETE)
	{
		std::cerr << "p3IdServiceVEG::getGroupList() ERROR Status Incomplete" << std::endl;
		return false;
	}
...
bool p3IdServiceVEG::getMsgList(           const uint32_t &token, std::list<std::string> &msgIds)
{
	uint32_t status;
	uint32_t reqtype;
	uint32_t anstype;
	...
	if (anstype != RS_TOKREQ_ANSTYPE_LIST)
	{
		std::cerr << "p3IdServiceVEG::getMsgList() ERROR AnsType Wrong" << std::endl;
		return false;
	}
	
	if ((reqtype != GXS_REQUEST_TYPE_MSGS) && (reqtype != GXS_REQUEST_TYPE_MSGRELATED))
	{
		std::cerr << "p3IdServiceVEG::getMsgList() ERROR ReqType Wrong" << std::endl;
		return false;
	}
	
	if (status != GXS_REQUEST_STATUS_COMPLETE)
	{
		std::cerr << "p3IdServiceVEG::getMsgList() ERROR Status Incomplete" << std::endl;
		return false;
	}
	...
	return ans;
        ...
        /* Generic Summary */
bool p3IdServiceVEG::getGroupSummary(      const uint32_t &token, std::list<RsGroupMetaData> &groupInfo)
{
	uint32_t status;
	uint32_t reqtype;
	uint32_t anstype;
	...
	{
		std::cerr << "p3IdServiceVEG::getGroupSummary() ERROR AnsType Wrong" << std::endl;
		return false;
	}
	
	if (reqtype != GXS_REQUEST_TYPE_GROUPS)
	{
		std::cerr << "p3IdServiceVEG::getGroupSummary() ERROR ReqType Wrong" << std::endl;
		return false;
	}
	
	if (status != GXS_REQUEST_STATUS_COMPLETE)
	{
		std::cerr << "p3IdServiceVEG::getGroupSummary() ERROR Status Incomplete" << std::endl;
		return false;
	}
...
bool p3IdServiceVEG::getMsgSummary(        const uint32_t &token, std::list<RsMsgMetaData> &msgInfo)
{
	uint32_t status;
	uint32_t reqtype;
	uint32_t anstype;
	...
	if (anstype != RS_TOKREQ_ANSTYPE_SUMMARY)
	{
		std::cerr << "p3IdServiceVEG::getMsgSummary() ERROR AnsType Wrong" << std::endl;
		return false;
	}
	
	if ((reqtype != GXS_REQUEST_TYPE_MSGS) && (reqtype != GXS_REQUEST_TYPE_MSGRELATED))
	{
		std::cerr << "p3IdServiceVEG::getMsgSummary() ERROR ReqType Wrong" << std::endl;
		return false;
	}
	
	if (status != GXS_REQUEST_STATUS_COMPLETE)
	{
		std::cerr << "p3IdServiceVEG::getMsgSummary() ERROR Status Incomplete" << std::endl;
		return false;
	}
...
bool p3IdServiceVEG::getGroupData(const uint32_t &token, RsIdGroup &group)
{
	uint32_t status;
	uint32_t reqtype;
	uint32_t anstype;
	...
	{
		std::cerr << "p3IdServiceVEG::getGroupData() ERROR AnsType Wrong" << std::endl;
		return false;
	}
	
	if (reqtype != GXS_REQUEST_TYPE_GROUPS)
	{
		std::cerr << "p3IdServiceVEG::getGroupData() ERROR ReqType Wrong" << std::endl;
		return false;
	}
	
	if (status != GXS_REQUEST_STATUS_COMPLETE)
	{
		std::cerr << "p3IdServiceVEG::getGroupData() ERROR Status Incomplete" << std::endl;
		return false;
	}
...
bool p3IdServiceVEG::getMsgData(const uint32_t &token, RsIdMsg &msg)
{
	uint32_t status;
	uint32_t reqtype;
	uint32_t anstype;
	...
	if (anstype != RS_TOKREQ_ANSTYPE_DATA)
	{
		std::cerr << "p3IdServiceVEG::getMsgData() ERROR AnsType Wrong" << std::endl;
		return false;
	}
	
	if ((reqtype != GXS_REQUEST_TYPE_MSGS) && (reqtype != GXS_REQUEST_TYPE_MSGRELATED))
	{
		std::cerr << "p3IdServiceVEG::getMsgData() ERROR ReqType Wrong" << std::endl;
		return false;
	}
	
	if (status != GXS_REQUEST_STATUS_COMPLETE)
	{
		std::cerr << "p3IdServiceVEG::getMsgData() ERROR Status Incomplete" << std::endl;
		return false;
	}
	...
	return ans;
        ...
        /* Poll */
uint32_t p3IdServiceVEG::requestStatus(const uint32_t token)
{
	uint32_t status;
	uint32_t reqtype;
	uint32_t anstype;
	...
	return status;
        ...
        /* Cancel Request */
bool p3IdServiceVEG::cancelRequest(const uint32_t &token)
{
	return clearRequest(token);
}

        //////////////////////////////////////////////////////////////////////////////
bool p3IdServiceVEG::setMessageStatus(const std::string &msgId, const uint32_t status, const uint32_t statusMask)
{
	return mIdProxy->setMessageStatus(msgId, status, statusMask);
}

bool p3IdServiceVEG::setGroupStatus(const std::string &groupId, const uint32_t status, const uint32_t statusMask)
{
	return mIdProxy->setGroupStatus(groupId, status, statusMask);
}

bool p3IdServiceVEG::setGroupSubscribeFlags(const std::string &groupId, uint32_t subscribeFlags, uint32_t subscribeMask)
{
	return mIdProxy->setGroupSubscribeFlags(groupId, subscribeFlags, subscribeMask);
}

bool p3IdServiceVEG::setMessageServiceString(const std::string &msgId, const std::string &str)
{
        return mIdProxy->setMessageServiceString(msgId, str);
}

bool p3IdServiceVEG::setGroupServiceString(const std::string &grpId, const std::string &str)
{
        return mIdProxy->setGroupServiceString(grpId, str);
}


bool p3IdServiceVEG::groupRestoreKeys(const std::string &groupId)
{
	return false;
}

bool p3IdServiceVEG::groupShareKeys(const std::string &groupId, std::list<std::string>& peers)
{
	return false;
}
...
std::string p3IdServiceVEG::genRandomId()
{
	std::string randomId;
	for(int i = 0; i < 20; i++)
	{
...
bool    p3IdServiceVEG::createGroup(uint32_t &token, RsIdGroup &group, bool isNew)
{
	...
	{
		std::cerr << "p3IdServiceVEG::createGroup() Group with existing Id... dropping";
		std::cerr << std::endl;
		return false;
	}
	...
	std::cerr << "p3IdServiceVEG::createGroup() Generating Request Token: " << token << std::endl;
	storeRequest(token, ansType, opts, GXS_REQUEST_TYPE_GROUPS, groupIds);
...
bool    p3IdServiceVEG::createMsg(uint32_t &token, RsIdMsg &msg, bool isNew)
{
	if (msg.mMeta.mGroupId.empty())
	{
		/* new photo */
		std::cerr << "p3IdServiceVEG::createMsg() Missing MsgID";
		std::cerr << std::endl;
		return false;
	}
	...
	{
		std::cerr << "p3IdServiceVEG::createMsg() New Msg";
		std::cerr << std::endl;
	...
	{
		std::cerr << "p3IdServiceVEG::createMsg() Modified Msg";
		std::cerr << std::endl;
	...
	std::cerr << "p3IdServiceVEG::createMsg() GroupId: " << msg.mMeta.mGroupId;
	std::cerr << std::endl;
	std::cerr << "p3IdServiceVEG::createMsg() MsgId: " << msg.mMeta.mMsgId;
	std::cerr << std::endl;
	std::cerr << "p3IdServiceVEG::createMsg() OrigMsgId: " << msg.mMeta.mOrigMsgId;
	std::cerr << std::endl;
	...
	std::cerr << "p3IdServiceVEG::createMsg() Generating Request Token: " << token << std::endl;
	storeRequest(token, ansType, opts, GXS_REQUEST_TYPE_MSGRELATED, msgIds);
...
/* details are updated  */
bool p3IdServiceVEG::updateIdentity(RsIdData &data)
{
...
bool p3IdServiceVEG::updateOpinion(RsIdOpinion &opinion)
{
	if (opinion.mKeyId.empty())
	{
		/* new photo */
		std::cerr << "p3IdServiceVEG::updateOpinion() Missing KeyId";
		std::cerr << std::endl;
		return false;
	}
	...
	{
		std::cerr << "p3IdServiceVEG::updateOpinion() Missing PeerId";
		std::cerr << std::endl;
		return false;
	}

	std::cerr << "p3IdServiceVEG::updateOpinion() KeyId: " << opinion.mKeyId;
	std::cerr << std::endl;
	std::cerr << "p3IdServiceVEG::updateOpinion() PeerId: " << opinion.mPeerId;
	std::cerr << std::endl;
...
void p3IdServiceVEG::generateDummyData()
{
	RsStackMutex stack(mIdMtx); /********** STACK LOCKED MTX ******/
	...
	for(it = gpgids.begin(); it != gpgids.end(); it++)
	{
		...
		for(i = 0; i < nIds; i++)
		{
			...
			{
				std::cerr << "p3IdServiceVEG::generateDummyData() missing" << std::endl;
				std::cerr << std::endl;
...
int	p3IdServiceVEG::background_tick()
{
	std::cerr << "p3IdServiceVEG::background_tick()";
	std::cerr << std::endl;
...
bool p3IdServiceVEG::background_checkTokenRequest()
{
	uint32_t token = 0;
	uint32_t phase = 0;
	{
...
bool p3IdServiceVEG::background_requestGroups()
{
	std::cerr << "p3IdServiceVEG::background_requestGroups()";
	std::cerr << std::endl;
...

	{
		...
		if (mBgProcessing)
		{
			std::cerr << "p3IdServiceVEG::background_requestGroups() ERROR Already processing, Skip this cycle";
			std::cerr << std::endl;
			return false;
		}
...
bool p3IdServiceVEG::background_requestNewMessages()
{
	std::cerr << "p3IdServiceVEG::background_requestNewMessages()";
	std::cerr << std::endl;
	...
	{
		std::cerr << "p3IdServiceVEG::background_requestNewMessages() ERROR No Group List";
		std::cerr << std::endl;
		background_cleanup();
		return false;
	}
...
bool p3IdServiceVEG::background_processNewMessages()
{
	std::cerr << "p3IdServiceVEG::background_processNewMessages()";
	std::cerr << std::endl;
	...
	{
		std::cerr << "p3IdServiceVEG::background_processNewMessages() ERROR No New Msgs";
		std::cerr << std::endl;
		background_cleanup();
		return false;
	}
	...
	{
		std::cerr << "p3IdServiceVEG::background_processNewMessages() new MsgId: " << it->mMsgId;
		std::cerr << std::endl;
		...
		{
			std::cerr << "p3IdServiceVEG::background_processNewMessages() ERROR missing GroupId: ";
		...
		{
			std::cerr << "p3IdServiceVEG::background_processNewMessages() Group Already marked FULL_CALC";
			std::cerr << std::endl;
		...
		if (it->mMsgId != it->mOrigMsgId)
		{
			...
			std::cerr << "p3IdServiceVEG::background_processNewMessages() Update, mark for FULL_CALC";
			std::cerr << std::endl;
		...
		else
		{
			...
			std::cerr << "p3IdServiceVEG::background_processNewMessages() NewOpt, Try Inc Calc";
			std::cerr << std::endl;
			...
			{
				/* error */
				std::cerr << "p3IdServiceVEG::background_processNewMessages() ERROR Extracting";
				std::cerr << std::endl;
			}

			/* do calcs */
			std::cerr << "p3IdServiceVEG::background_processNewMessages() Extracted: ";
			std::cerr << std::endl;

			/* store it back in */
			std::cerr << "p3IdServiceVEG::background_processNewMessages() Stored: ";
			std::cerr << std::endl;

			if (!encodeIdGroupCache(serviceString, ssData))
			{
				/* error */
				std::cerr << "p3IdServiceVEG::background_processNewMessages() ERROR Storing";
				std::cerr << std::endl;
			}
		}
	}
...

	{
		...
		std::cerr << "p3IdServiceVEG::background_processNewMessages() Checking Groups for Calc Type";
		std::cerr << std::endl;
		...
		for(mit = mBgGroupMap.begin(); mit != mBgGroupMap.end(); mit++)
		{
			if (mit->second.mGroupStatus & ID_LOCAL_STATUS_FULL_CALC_FLAG)
			{
				std::cerr << "p3IdServiceVEG::background_processNewMessages() FullCalc for: ";
			...
			{
				std::cerr << "p3IdServiceVEG::background_processNewMessages() IncCalc done for: ";
				std::cerr << mit->second.mGroupId;
				std::cerr << std::endl;
			...
			else
			{
				...
				std::cerr << "p3IdServiceVEG::background_processNewMessages() ERROR for: ";
				std::cerr << mit->second.mGroupId;
				std::cerr << std::endl;
			}
		}
...
bool p3IdServiceVEG::encodeIdGroupCache(std::string &str, const IdGroupServiceStrData &data)
{
...
bool p3IdServiceVEG::extractIdGroupCache(std::string &str, IdGroupServiceStrData &data)
{
	...
	{
		std::cerr << "p3IdServiceVEG::extractIdGroupCache() Failed to extract Two Parts";
		std::cerr << std::endl;
		return false;
	}

	std::cerr << "p3IdServiceVEG::extractIdGroupCache() pgpline: " << pgpline;
	std::cerr << std::endl;
	std::cerr << "p3IdServiceVEG::extractIdGroupCache() scoreline: " << scoreline;
	std::cerr << std::endl;
	
	std::string pgptmp = pgpline;
	if (pgptmp.length() > 5)
	{
		std::cerr << "p3IdServiceVEG::extractIdGroupCache() Believe to have pgpId: " << pgptmp;
		std::cerr << std::endl;
	...
	{
		std::cerr << "p3IdServiceVEG::extractIdGroupCache() Think pgpId Invalid";
		std::cerr << std::endl;
	...
	std::cerr << "p3IdServiceVEG::extractIdGroupCache() Failed to extract scores";
	std::cerr << std::endl;
...
bool p3IdServiceVEG::background_FullCalcRequest()
{
	/* 
...
bool p3IdServiceVEG::background_processFullCalc()
{
	std::cerr << "p3IdServiceVEG::background_processFullCalc()";
	std::cerr << std::endl;
	...
	{
		std::cerr << "p3IdServiceVEG::background_processFullCalc() Msg:";
		std::cerr << msg;
		std::cerr << std::endl;
	...
	if (validmsgs)
	{
		...
		if (!encodeIdGroupCache(serviceString, ssData))
		{
			std::cerr << "p3IdServiceVEG::background_updateVoteCounts() Failed to encode Votes";
			std::cerr << std::endl;
		}
		else
		{
			std::cerr << "p3IdServiceVEG::background_updateVoteCounts() Encoded String: " << serviceString;
			std::cerr << std::endl;
			/* store new result */
...
bool p3IdServiceVEG::background_cleanup()
{
	std::cerr << "p3IdServiceVEG::background_cleanup()";
	std::cerr << std::endl;
