...
const uint32_t P3CONNMGR_TCP_DEFAULT_DELAY = 3;
const uint32_t P3CONNMGR_UDP_DEFAULT_DELAY = 3;
...
// times a peer stay in available state when not connected
#define MAX_AVAIL_PERIOD 230
#define MIN_RETRY_PERIOD 140

// seconds
#define MAX_RANDOM_ATTEMPT_OFFSET 6

void printConnectState(std::ostream &out, peerConnectState &peer);

peerConnectAddress::peerConnectAddress() : delay(0), period(0), type(0),
	flags(0), ts(0), bandwidth(0), domain_port(0)
{
...
peerAddrInfo::peerAddrInfo() : found(false), type(0), ts(0) {}

peerConnectState::peerConnectState() : connecttype(0), lastavailable(0),
	lastattempt(0), name(""), state(0), actions(0), source(0),
	inConnAttempt(0), wasDeniedConnection(false), deniedTS(false),
...
bool p3LinkMgrIMPL::isOnline(const RsPeerId &ssl_id)
{
	...
	if (it->second.state & RS_PEER_S_CONNECTED) return true;
	return false;
}
...
uint32_t p3LinkMgrIMPL::getLinkType(const RsPeerId &ssl_id)
{
	...
	std::map<RsPeerId, peerConnectState>::iterator it;
	it = mFriendList.find(ssl_id);
	if (it == mFriendList.end()) return 0;
	if (it->second.state & RS_PEER_S_CONNECTED) return it->second.linkType;
	return 0;
}
...
void p3LinkMgrIMPL::setFriendVisibility(const RsPeerId &id, bool isVisible)
{
	/* set visibility */
	{
		...
		{
			std::cerr << "p3LinkMgrIMPL::setFriendVisibility() ERROR peer unknown: "
					  << id << std::endl;
			return;
		}

		// no change in state
		if (it->second.dhtVisible == isVisible) return;

		it->second.dhtVisible = isVisible;

		// dont worry about it
		if (it->second.state & RS_PEER_S_CONNECTED) return;
	}
...
void p3LinkMgrIMPL::statusTick()
{
...

	{
		...
		for(it = mFriendList.begin(); it != mFriendList.end(); ++it)
		{
			if (it->second.state & RS_PEER_S_CONNECTED) continue;
...
bool p3LinkMgrIMPL::connectAttempt(const RsPeerId &id, sockaddr_storage &raddr,
								   sockaddr_storage &proxyaddr,
								   ...
								   std::string &domain_addr, uint16_t &domain_port)
{
	...
	if (it->second.state & RS_PEER_S_CONNECTED)
	{
		...
		std::cerr << "p3LinkMgrIMPL::connectAttempt() " << id.toStdString()
				  << " Already FLAGGED as connected, but allowing anyway!"
				  << std::endl;
	}
#endif
...
#define LOW_BANDWIDTH_LIMIT	5001		// 5kb
	...
	std::cerr << "p3LinkMgrIMPL::connectAttempt() Setting up LinkType" << type << std::endl;
#endif
...
#if defined(LINKMGR_DEBUG_LINKTYPE) || defined(LINKMGR_DEBUG)
	std::cerr << "p3LinkMgrIMPL::connectAttempt() connType: " << connType
			  << " final LinkType: " << it->second.linkType
			  << " found an address: id: " << id
			  << " laddr: " << sockaddr_storage_tostring(raddr) << " delay: "
			  << delay << " period: " << period << " type: " << type
			  << "p3LinkMgrIMPL::connectAttempt() set LinkType to: "
			  << it->second.linkType << std::endl;
...
/**
 * TODO: 2016/01/06 Move documentation to header
 * Update state,
 ...
 *
 */
bool p3LinkMgrIMPL::connectResult(const RsPeerId &id, bool success,
								  bool isIncomingConnection, uint32_t flags,
								  const sockaddr_storage &remote_peer_address)
{
...

	{
		...
		if (success)
		{
...
#ifdef LINKMGR_DEBUG
			std::cerr << "p3LinkMgrIMPL::connectResult() Connect! id: " << id
					  << " Success: " << success << " flags: " << flags
					  << " remote IP = " <<  sockaddr_storage_iptostring(remote_peer_address) << std::endl;
#endif
			...
			if ( it->second.inConnAttempt &&
					sockaddr_storage_same(
					 it->second.currentConnAddrAttempt.addr,
					 remote_peer_address) )
			{
				updatePeerAddr = true;
#ifdef LINKMGR_DEBUG
		...
		else
		{
			...
			if (it->second.dhtVisible) doDhtAssist = true;
		}
	}
...
#ifdef LINKMGR_DEBUG
		std::cerr << "p3LinkMgrIMPL::connectResult() Success and we initiated connection... Updating Address" << std::endl;
#endif
		mPeerMgr->updateCurrentAddress(id, raddr);
	}

	if (updateLastContact) mPeerMgr->updateLastContact(id);
	...
	{
#ifdef LINKMGR_DEBUG
		std::cerr << "p3LinkMgrIMPL::connectResult() Sending Feedback for UDP connection" << std::endl;
#endif
		if (success)
		{
#ifdef LINKMGR_DEBUG
			std::cerr << "p3LinkMgrIMPL::connectResult() UDP Update CONNECTED to: " << id << std::endl;
#endif
		...
		{
#ifdef LINKMGR_DEBUG
			std::cerr << "p3LinkMgrIMPL::connectResult() UDP Update FAILED to: " << id << std::endl;
#endif
	...
	{
#ifdef LINKMGR_DEBUG
		std::cerr << "p3LinkMgrIMPL::connectResult() Success switching off DhtAssist for friend: " << id << std::endl;
#endif
		...
		if (sockaddr_storage_isExternalNet(remote_peer_address))
		{
			mNetMgr->netAssistKnownPeer(id,remote_peer_address,
										NETASSIST_KNOWN_PEER_FRIEND |
										NETASSIST_KNOWN_PEER_ONLINE);
		}
	}
	else
	{
		...
		{
#ifdef LINKMGR_DEBUG
			std::cerr << "p3LinkMgrIMPL::connectResult() Fail, Enabling DhtAssist for: " << id << std::endl;
#endif
			mNetMgr->netAssistFriend(id,true) ;
		}
#ifdef LINKMGR_DEBUG
		else std::cerr << "p3LinkMgrIMPL::connectResult() Fail, No DhtAssist, as No DHT visibility for: " << id << std::endl;
#endif

		/* inform NetMgr that this peer is offline */
		mNetMgr->netAssistKnownPeer(id,remote_peer_address,
									NETASSIST_KNOWN_PEER_FRIEND |
									NETASSIST_KNOWN_PEER_OFFLINE);
	}
...
void p3LinkMgrIMPL::notifyDeniedConnection(const RsPgpId& gpgid,
										   const RsPeerId& sslid,
										   const std::string& sslcn,
										   const sockaddr_storage & /*addr*/,
										   bool incoming)
{
	std::cerr << "p3LinkMgrIMPL::notifyDeniedConnection()" << " pgpid: "
			  << gpgid << " sslid: " << sslid << " sslcn: " << sslcn << std::endl;
	...
	{
#ifdef LINKMGR_DEBUG
		std::cerr << "p3LinkMgrIMPL::notifyDeniedConnection() of NON-FRIEND: "
				  << sslid << std::endl;
#endif
		return;
	}
...
#ifdef LINKMGR_DEBUG
		std::cerr << "p3LinkMgrIMPL::notifyDeniedConnection() Denied"
				  << "In Connection Attempt" << std::endl;
#endif
	}
	else
	{
...
#ifdef LINKMGR_DEBUG
		std::cerr << "p3LinkMgrIMPL::notifyDeniedConnection() Denied"
				  << "NOT In Connection Attempt" << std::endl;
#endif
	}
}
...
{
	// HACKED UP FIX // TODO: 2016/01/06 what this is supposed to mean?
	...
	
	{
		...
		{
#ifdef LINKMGR_DEBUG
			std::cerr << "p3LinkMgrIMPL::peerStatus() Update From DHT:" << std::endl;
#endif
		...
		{
#ifdef LINKMGR_DEBUG
			std::cerr << "p3LinkMgrIMPL::peerStatus() Update From DISC:" << std::endl;
#endif
		...
		else if (source == RS_CB_PERSON)
		{
			...
			*/
#ifdef LINKMGR_DEBUG
			std::cerr << "p3LinkMgrIMPL::peerStatus() Update From PERSON:" << std::endl;
#endif
			...
			if (flags & RS_NET_FLAGS_EXTERNAL_ADDR) peerNetMode = RS_NET_MODE_EXT;
			else if (flags & RS_NET_FLAGS_STABLE_UDP) peerNetMode = RS_NET_MODE_UDP;
			else peerNetMode = RS_NET_MODE_UNREACHABLE;

			/* always update VIS status */
			if (flags & RS_NET_FLAGS_USE_DISC) peer_vs_disc = RS_VS_DISC_FULL;
			else peer_vs_disc = RS_VS_DISC_OFF;

			if (flags & RS_NET_FLAGS_USE_DHT) peer_vs_dht = RS_VS_DHT_FULL;
			else peer_vs_dht = RS_VS_DHT_OFF;
		}

		// Determine Reachability (only advisory)
		if (ownNetMode & RS_NET_MODE_UDP)
			if ((details.type & RS_NET_CONN_UDP_DHT_SYNC) ||
					(details.type & RS_NET_CONN_TCP_EXTERNAL)) // reachable!
				it->second.state &= (~RS_PEER_S_UNREACHABLE);
			else it->second.state |= RS_PEER_S_UNREACHABLE;
		else if (ownNetMode & RS_NET_MODE_UNREACHABLE)
			if (details.type & RS_NET_CONN_TCP_EXTERNAL) // reachable!
				it->second.state &= (~RS_PEER_S_UNREACHABLE);
			else it->second.state |= RS_PEER_S_UNREACHABLE;
		else it->second.state &= (~RS_PEER_S_UNREACHABLE);
...
#ifdef LINKMGR_DEBUG
	std::cerr << "p3LinkMgrIMPL::peerStatus()" << " id: " << id << " type: "
			  << type << " flags: " << flags << " source: " << source
			  << " addrs: " << std::endl;
	std::string out;
	addrs.printAddrs(out);
	std::cerr << out << std::endl;
...
#if !(defined(P3CONNMGR_NO_TCP_CONNECTIONS) || defined(P3CONNMGR_NO_AUTO_CONNECTION ))
	if (newAddrs) retryConnectTCP(id);
#endif // !(defined(P3CONNMGR_NO_TCP_CONNECTIONS) || defined(P3CONNMGR_NO_AUTO_CONNECTION ))
...
void p3LinkMgrIMPL::peerConnectRequest(const RsPeerId& id,
									   const sockaddr_storage &raddr,
									   const sockaddr_storage &proxyaddr,
									   const sockaddr_storage &srcaddr,
									   uint32_t source, uint32_t flags,
									   uint32_t delay, uint32_t bandwidth)
{
#ifdef LINKMGR_DEBUG
	std::cerr << "p3LinkMgrIMPL::peerConnectRequest() id: " << id << " raddr: "
			  << sockaddr_storage_tostring(raddr) << " proxyaddr: "
			  << sockaddr_storage_tostring(proxyaddr) << " srcaddr: "
			  << sockaddr_storage_tostring(srcaddr) << " source: " << source
			  << " flags: " << flags << " delay: " << delay << " bandwidth: "
			  << bandwidth << std::endl;
#endif
	/******************** TCP PART *****************************/
	...
	if (source == RS_CB_DHT)
	{
...
#ifdef LINKMGR_DEBUG
			std::cerr << "p3LinkMgrIMPL::peerConnectRequest() DHT says Online ==> so try TCP" << std::endl;
#endif
			{
				RS_STACK_MUTEX(mLinkMtx);
		...
		else
		{
			// UDP Attempt!
#ifdef LINKMGR_DEBUG
			std::cerr << "p3LinkMgrIMPL::peerConnectRequest() DHT says CONNECT ==> tryConnectUDP()" << std::endl;
#endif
...
bool p3LinkMgrIMPL::tryConnectUDP(const RsPeerId &id,
								  const sockaddr_storage &rUdpAddr,
								  const sockaddr_storage &proxyaddr,
								  const sockaddr_storage &srcaddr,
								  uint32_t flags, uint32_t delay,
								  uint32_t bandwidth)

{
#ifdef DISABLE_UDP_CONNECTIONS
	std::cerr << "p3LinkMgrIMPL::tryConnectUDP() UDP CONNECTIONS DISABLED id: " << id << std::endl;
	mNetMgr->netAssistStatusUpdate(id, NETMGR_DHT_FEEDBACK_CONN_FAILED);
	return false;
#endif
	...
	{
#ifdef LINKMGR_DEBUG
		std::cerr << "p3LinkMgrIMPL::tryConnectUDP() isHidden(): no connection attempts for : " << id << std::endl;
#endif
		return false;
	}
	...
	{
#ifdef LINKMGR_DEBUG
		std::cerr << "p3LinkMgrIMPL::retryConnectUDP() Failed, connecting to own id" << std::endl;
#endif
		return false;
	}
	...
	{
#ifdef LINKMGR_DEBUG
		std::cerr << "Adding udp connection attempt: " << "Addr: " << sockaddr_storage_tostring(rUdpAddr) << std::endl;
#endif
...
{
#ifdef LINKMGR_DEBUG
	std::cerr << "p3LinkMgrIMPL::locked_CheckPotentialAddr(" << sockaddr_storage_tostring(addr) << ", " << age << ")" << std::endl;
#endif
	...
	{
#ifdef LINKMGR_DEBUG
		std::cerr << "p3LinkMgrIMPL::locked_CheckPotentialAddr() REJECTING - TOO OLD" << std::endl;
#endif
		return false;
	}
	...
	{
#ifdef LINKMGR_DEBUG
		std::cerr << "p3LinkMgrIMPL::locked_CheckPotentialAddr() REJECTING - INVALID" << std::endl;
#endif
		return false;
	}
	...
	for(it = mBannedIpList.begin(); it != mBannedIpList.end(); ++it)
	{
		...
		{
#ifdef LINKMGR_DEBUG
			std::cerr << "p3LinkMgrIMPL::locked_CheckPotentialAddr() REJECTING - ON BANNED IPLIST" << std::endl;
#endif
			return false;
		}
	}
	...
	{
#ifdef LINKMGR_DEBUG
		std::cerr << "p3LinkMgrIMPL::locked_CheckPotentialAddr() adding to local Banned IPList" << std::endl;
#endif
		return false;
	}
...
void p3LinkMgrIMPL::locked_ConnectAttempt_SpecificAddress(
		peerConnectState *peer, const sockaddr_storage &remoteAddr)
{
#ifdef LINKMGR_DEBUG
	std::cerr << "p3LinkMgrIMPL::locked_ConnectAttempt_SpecificAddresses()" << std::endl;
#endif
	if (locked_CheckPotentialAddr(remoteAddr, 0))
	{
#ifdef LINKMGR_DEBUG
		std::cerr << "p3LinkMgrIMPL::locked_ConnectAttempt_SpecificAddresses()"
				  << " Adding tcp connection attempt Addr: "
				  << sockaddr_storage_tostring(remoteAddr) << std::endl;
#endif
		peerConnectAddress pca;
		pca.addr = remoteAddr;
...
void p3LinkMgrIMPL::locked_ConnectAttempt_CurrentAddresses(
		peerConnectState *peer, const sockaddr_storage &localAddr,
		const sockaddr_storage &serverAddr)
{
#ifdef LINKMGR_DEBUG
	std::cerr << "p3LinkMgrIMPL::locked_ConnectAttempt_CurrentAddresses()" << std::endl;
#endif
	...
	if (locked_CheckPotentialAddr(localAddr, 0))
	{
		...
		std::cerr << "p3LinkMgrIMPL::locked_ConnectAttempt_CurrentAddresses() "
				  << "Adding tcp connection attempt, Current Local Addr: "
				  << sockaddr_storage_tostring(localAddr) << std::endl;
#endif
		peerConnectAddress pca;
		pca.addr = localAddr;
...
#ifdef LINKMGR_DEBUG
		std::cerr << "p3LinkMgrIMPL::locked_ConnectAttempt_CurrentAddresses() "
				  << "Adding tcp connection attempt, Current Ext Addr: "
				  << sockaddr_storage_tostring(serverAddr) << std::endl;
#endif
		peerConnectAddress pca;
		pca.addr = serverAddr;
		pca.type = RS_NET_CONN_TCP_EXTERNAL;
...
void p3LinkMgrIMPL::locked_ConnectAttempt_HistoricalAddresses(
		peerConnectState *peer,
		const pqiIpAddrSet &ipAddrs)
{
...
#ifdef LINKMGR_DEBUG
	std::cerr << "p3LinkMgrIMPL::locked_ConnectAttempt_HistoricalAddresses()" << std::endl;
#endif
	for(ait = ipAddrs.mLocal.mAddrs.begin();  ait != ipAddrs.mLocal.mAddrs.end(); ++ait)
	{
		if (locked_CheckPotentialAddr(ait->mAddr, now - ait->mSeenTime))
		{
			...
			std::cerr << "p3LinkMgrIMPL::locked_ConnectAttempt_HistoricalAddresses() "
					  << "Adding tcp connection attempt, Local Addr: "
					  << sockaddr_storage_tostring(ait->mAddr) << std::endl;
#endif
			peerConnectAddress pca;
			pca.addr = ait->mAddr;
			pca.type = RS_NET_CONN_TCP_LOCAL;
...
#ifdef LINKMGR_DEBUG
			std::cerr << "p3LinkMgrIMPL::locked_ConnectAttempt_HistoricalAddresses() "
					  << "Adding tcp connection attempt, Ext Addr: "
					  << sockaddr_storage_tostring(ait->mAddr) << std::endl;
#endif
			peerConnectAddress pca;
			pca.addr = ait->mAddr;
	...
	if (!dyndns.empty() && port)
	{
		...
		if(mDNSResolver->getIPAddressFromString(dyndns, addr))
		{
			...
			std::cerr << "p3LinkMgrIMPL::locked_ConnectAttempt_AddDynDNS() "
					  << "Adding tcp connection attempt, DynDNS Addr: "
					  << sockaddr_storage_iptostring(addr) << ":" << port << std::endl;
#endif
			peerConnectAddress pca;
...
#ifdef LINKMGR_DEBUG
		else std::cerr << "p3LinkMgrIMPL::locked_ConnectAttempt_AddDynDNS() "
					   << "DNSResolver hasn't found addr yet" << std::endl;
	}
	else std::cerr << "p3LinkMgrIMPL::locked_ConnectAttempt_AddDynDNS() Address("
				   << dyndns << ") or Port(" << port << ") NULL ignoring" << std::endl;
#else // LINKMGR_DEBUG
	}
#endif // LINKMGR_DEBUG
}
...
void p3LinkMgrIMPL::locked_ConnectAttempt_ProxyAddress(
		peerConnectState *peer, const uint32_t type, // TODO: 2016/01/07 const on uint32_t doesn't make much sense
		const sockaddr_storage &proxy_addr, const std::string &domain_addr,
		uint16_t domain_port)
{
#ifdef LINKMGR_DEBUG
	...
	default:
		...
		std::cerr << "p3LinkMgrIMPL::locked_ConnectAttempt_ProxyAddress()"
				  << "hidden type of addr: " << domain_addr
				  << " is unkown -> THIS SHOULD NEVER HAPPEN!" << std::endl
				  << " - peer : " << peer->id << "(" << peer->name << ")"
				  << " - proxy: " << sockaddr_storage_tostring(proxy_addr)
				  << " - addr : " << domain_addr << ":" << domain_port << std::endl;
...
bool p3LinkMgrIMPL::addAddressIfUnique(std::list<peerConnectAddress> &addrList,
									   peerConnectAddress &pca, bool pushFront)
{
	/* iterate through the list, and make sure it isn't already
	 * in the list
	 */
#ifdef LINKMGR_DEBUG
	std::cerr << "p3LinkMgrIMPL::addAddressIfUnique() Checking Address: " << sockaddr_storage_iptostring(pca.addr) << std::endl;
#endif
	...
	for(it = addrList.begin(); it != addrList.end(); ++it)
	{
		...
		{
#ifdef LINKMGR_DEBUG
			std::cerr << "p3LinkMgrIMPL::addAddressIfUnique() Discarding Duplicate Address" << std::endl;
#endif
			return false;
		}
	}
...
#ifdef LINKMGR_DEBUG
	std::cerr << "p3LinkMgrIMPL::addAddressIfUnique() Adding New Address" << std::endl;
#endif

	if (pushFront) addrList.push_front(pca);
	else addrList.push_back(pca);
...
#ifdef LINKMGR_DEBUG
		std::cerr << "p3LinkMgrIMPL::locked_ConnectAttempt_Complete() Already"
				  << "in CONNECT ATTEMPT Remaining ConnAddr Count: "
				  << peer->connAddrs.size() << std::endl;
#endif
		return true;
	}
...
int p3LinkMgrIMPL::addFriend(const RsPeerId &id, bool isVisible)
{
...

	{
...
#ifdef LINKMGR_DEBUG
		std::cerr << "p3LinkMgrIMPL::addFriend(" << id << "," << isVisible << ")" << std::endl;
#endif
		...
		if (it != mFriendList.end())
		{
			std::cerr << "p3LinkMgrIMPL::addFriend() ERROR, friend already exists : " << id << std::endl;
			return 0;
		}
...
int p3LinkMgrIMPL::removeFriend(const RsPeerId &id)
{
...

	{
...
#ifdef LINKMGR_DEBUG
		std::cerr << "p3LinkMgrIMPL::removeFriend(" << id << ")" << std::endl;
#endif
		...
		if (it == mFriendList.end())
		{
			std::cerr << "p3LinkMgrIMPL::removeFriend() ERROR, friend not there : " << id << std::endl;
			return 0;
		}
...
void p3LinkMgrIMPL::disconnectFriend(const RsPeerId& id)
{
	...
	for(std::list<pqiMonitor*>::const_iterator it(disconnect_clients.begin());
		it != disconnect_clients.end(); ++it)
...
void p3LinkMgrIMPL::printPeerLists(std::ostream &out)
{
	...
	out << "p3LinkMgrIMPL::printPeerLists() Friend List" << std::endl;
	std::map<RsPeerId, peerConnectState>::iterator it;
	for(it = mFriendList.begin(); it != mFriendList.end(); ++it)
		out << "\t SSL ID: " << it->second.id.toStdString()
			<< "\t State: " << it->second.state << std::endl;

	out << "p3LinkMgrIMPL::printPeerLists() Others List" << std::endl;
	for(it = mOthersList.begin(); it != mOthersList.end(); ++it)
		out << "\t SSL ID: " << it->second.id.toStdString()
