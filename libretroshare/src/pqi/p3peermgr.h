...
public:
	...
	virtual bool addFriend(const RsPeerId&ssl_id, const RsPgpId&gpg_id,
						   ...
						   uint16_t vsDht = RS_VS_DHT_FULL,
						   time_t lastContact = 0,
						   ServicePermissionFlags = ServicePermissionFlags(RS_NODE_PERM_DEFAULT));
	...
	virtual bool setLocalAddress(const RsPeerId &id, const sockaddr_storage &addr);
	virtual bool setExtAddress(const RsPeerId &id, const sockaddr_storage &addr);
	virtual bool setDynDNS(const RsPeerId &id, const std::string &dyndns);
	virtual bool addCandidateForOwnExternalAddress(const RsPeerId& from, const sockaddr_storage &addr);
	virtual bool getExtAddressReportedByFriends(sockaddr_storage& addr, uint8_t &isstable);
	...
	virtual bool getProxyAddress(const RsPeerId& ssl_id,
	...
	p3PeerMgrIMPL(const RsPeerId& ssl_own_id, const RsPgpId& gpg_own_id,
...
private:
	...
	sockaddr_storage mProxyServerAddressTor;
	sockaddr_storage mProxyServerAddressI2P;
