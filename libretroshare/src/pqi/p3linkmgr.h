...
public:
	...
	sockaddr_storage connectaddr; // current connection address. Can be local or external.
...
public:
	...
	/****************** Connections *******************/
	virtual bool connectAttempt(const RsPeerId &id, sockaddr_storage &raddr,
	                            sockaddr_storage &proxyaddr,
	                            sockaddr_storage &srcaddr, uint32_t &delay,
	                            uint32_t &period, uint32_t &type,
	                            uint32_t &flags, uint32_t &bandwidth,
	                            std::string &domain_addr, uint16_t &domain_port
	                            ) = 0;
	
	virtual bool connectResult(const RsPeerId &id, bool success,
	                           bool isIncomingConnection, uint32_t flags,
	                           const sockaddr_storage &remote_peer_address) = 0;

	virtual bool retryConnect(const RsPeerId &id) = 0;

	virtual void notifyDeniedConnection(const RsPgpId& gpgid,
	...
	/* Network Addresses */
	virtual bool setLocalAddress(const sockaddr_storage &addr) = 0;
	virtual bool getLocalAddress(sockaddr_storage &addr) = 0;
	...
	virtual bool checkPotentialAddr(const sockaddr_storage &addr, time_t age) = 0;
...
public:
	...
	/**
	 * @brief retryConnect push all available addresses onto the connect addr stack
	 * @param id
	 * @return
	 */
	...
	/**
	 * @brief notifyDeniedConnection from pqissl, when a connection failed due to security
	 * @param gpgid
	 * @param sslid
	 * @param sslcn
	 * @param incoming
	...
	/* Network Addresses */
	virtual bool setLocalAddress(const sockaddr_storage &addr);
	virtual bool getLocalAddress(sockaddr_storage &addr);
...
protected:
	...
	bool tryConnectUDP(const RsPeerId &id, const sockaddr_storage &rUdpAddr,
	                   const sockaddr_storage &proxyaddr,
	                   const sockaddr_storage &srcaddr,
	...
	void locked_ConnectAttempt_SpecificAddress(
	        peerConnectState *peer, const sockaddr_storage &remoteAddr);
	void locked_ConnectAttempt_CurrentAddresses(
	...
	void locked_ConnectAttempt_AddDynDNS(peerConnectState *peer,
	                                     std::string dyndns, uint16_t dynPort);
	void locked_ConnectAttempt_AddTunnel(peerConnectState *peer);
	void locked_ConnectAttempt_ProxyAddress(
	        peerConnectState *peer, const uint32_t type,
	        const sockaddr_storage &proxy_addr, const std::string &domain_addr,
	        uint16_t domain_port);

	bool locked_ConnectAttempt_Complete(peerConnectState *peer);

	bool locked_CheckPotentialAddr(const sockaddr_storage &addr, time_t age);
...
private:
	...
	bool mStatusChanged;
	sockaddr_storage mLocalAddress;
	...
	std::list<sockaddr_storage> mBannedIpList;
};
